...
import { corsOptions, logConnectionDetails } from './ngrokConfig.js';
...
// PRODUCTION CORS CONFIGURATION - Using centralized config from cloudflareConfig.js
...
// Also serve files from frontend/public for debugging
const frontendPublicPath = path.join(__dirname, '../../frontend/public');
if (fs.existsSync(frontendPublicPath)) {
    console.log(`[Server] Also serving debug files from: ${frontendPublicPath}`);
    app.use(express.static(frontendPublicPath, {
      maxAge: '0', // No caching for debug files
      etag: false  // No etag for debug files
    }));
}
  ...
  res.on('finish', () => {
    ...
    try {
      if (req.originalUrl.includes('/socket.io/') && !res.headersSent) {
        // Use centralized Cloudflare header function
        logConnectionDetails(res);
      }
    } catch (err) {
