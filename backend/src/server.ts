...
app.use((_req: Request, res: Response, next: NextFunction) => { // Mark req as unused
...
app.post('/api/water', (req: Request, res: Response) => { const { playerId } = req.body; if (playerId === undefined) return res.status(400).json({ error: 'Missing params' }); handleRequest(() => gameHandler.waterPlants(playerId), res, 'water'); }); // Fixed arity
...
// SSL Certificates
let sslOptions: https.ServerOptions | undefined = undefined; const keyPath = path.join(__dirname, '../certs/key.pem'); const certPath = path.join(__dirname, '../certs/cert.pem'); try { if (fs.existsSync(keyPath) && fs.existsSync(certPath)) { sslOptions = { key: fs.readFileSync(keyPath), cert: fs.readFileSync(certPath) }; console.log("[Server] SSL loaded."); } else { console.log("[Server] SSL not found. HTTP only."); } } catch (err) { console.error("[Server] Error reading SSL certs:", err); console.log("[Server] HTTP only."); }

// Start Server
const HTTP_PORT = process.env.PORT || 8080; const HTTPS_PORT = process.env.HTTPS_PORT || 8443;
http.createServer(app).listen(HTTP_PORT, () => { console.log(`🌙 Backend server listening at http://localhost:${HTTP_PORT}`); console.log(`--------------------------------------------------`); });
if (sslOptions) { https.createServer(sslOptions, app).listen(HTTPS_PORT, () => { console.log(`✨ Backend server (HTTPS) listening at https://localhost:${HTTPS_PORT}`); console.log(`--------------------------------------------------`); }); } else { console.log(` (HTTPS server not started)`); }
