import { ElementType, ItemType, ItemCategory, TarotCard } from 'coven-shared';
import { findCardById } from './tarotCardMocks';

// Map of brewing methods to element types (used in compatibility calculations)
const methodToElementMap: Record<string, ElementType> = {
  'Decoction': 'Fire',
  'Infusion': 'Water',
  'Distillation': 'Air',
  'Fermentation': 'Earth',
...
/**
 * Calculate compatibility between ingredients and brewing method or between a single card and a method
 * @param ingredientCardsOrCard Array of ingredient cards, a single card, or a single card + a card
 * @param methodOrCard Brewing method or a second card
 * @param optionalMethod Optional third parameter that's a brewing method
 * @returns Compatibility score (0-100)
 */
export function calculateCompatibility(
  ingredientCardsOrCard: TarotCard[] | TarotCard,
  methodOrCard: BrewingMethod | TarotCard,
  optionalMethod?: BrewingMethod
): number {
  // Case 1: Calculate compatibility between a single card and a brewing method
  if (!Array.isArray(ingredientCardsOrCard) && typeof methodOrCard === 'string') {
    const card = ingredientCardsOrCard;
    const method = methodOrCard;
    ...
    if (
      ...
      score += 20; // Strong category match
    }
    
    return Math.min(100, Math.max(0, score));
  }
  
  // Case 2: Calculate compatibility between two cards
  if (!Array.isArray(ingredientCardsOrCard) && typeof methodOrCard !== 'string') {
    const card1 = ingredientCardsOrCard;
    const card2 = methodOrCard;
    ...
    if (card1.element === card2.element) {
      ...
      (card1.element === 'Fire' && card2.element === 'Air') ||
      (card1.element === 'Air' && card2.element === 'Fire') ||
      (card1.element === 'Water' && card2.element === 'Earth') ||
      (card1.element === 'Earth' && card2.element === 'Water')
    ) {
      score += 10; // Complementary elements
    } else if (
      (card1.element === 'Fire' && card2.element === 'Water') ||
      (card1.element === 'Water' && card2.element === 'Fire') ||
      (card1.element === 'Air' && card2.element === 'Earth') ||
      (card1.element === 'Earth' && card2.element === 'Air')
    ) {
      score -= 10; // Opposing elements
    }
    
    // Category compatibility
    if (card1.category === card2.category) {
      score += 15; // Same category
    }
    ...
    // Additional brewing method compatibility if provided
    if (optionalMethod) {
      const methodElement = brewingMethodElements[optionalMethod];
      if (card1.element === methodElement) score += 5;
      if (card2.element === methodElement) score += 5;
    }
    
    return Math.min(100, Math.max(0, score));
  }
  
  // Case 3: Calculate compatibility between an array of cards and a brewing method
  const ingredientCards = ingredientCardsOrCard as TarotCard[];
  const method = methodOrCard as BrewingMethod;
